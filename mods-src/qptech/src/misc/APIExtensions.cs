...
namespace qptech.src.extensions
{
    /// <summary>
    /// This code by the amazing & helpful Capsup
    /// https://gitlab.com/vsmods-public/foundation/-/blob/master/Extensions/ApiExtensions.cs
    /// </summary>
    public static class ApiExtensions
    {
        ...
        public static TData LoadOrCreateDataFile<TData>(this ICoreAPI api, string filename) where TData : new()
        {
            ...
            catch (Exception e)
            {
                if (File.Exists(path))
                {
                    api.World.Logger.Log(EnumLogType.Error, $"Failed loading file '{path}' with error '{e}'!");
                    throw e;
                }
                else
                {
                    api.World.Logger.Log(EnumLogType.Error, $"Failed loading file '{path}' with error '{e}'. Will initialize new one...");
                }
            }
        ...
        public static void SaveDataFile<TData>(this ICoreAPI api, string filename, TData data) where TData : new()
        {
            ...
            {
                api.World.Logger.Log(EnumLogType.Error, $"Failed saving file '{path}' with error '{e}'!");
            }
        }
