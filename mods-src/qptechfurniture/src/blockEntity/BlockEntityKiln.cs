...
namespace QptechFurniture.src
{
    ...
    public interface IInKilnMeshSupplier
    {
        ...
        /// <param name="kilnModel"></param>
        /// <returns></returns>
        MeshData GetMeshWhenInKiln(ItemStack stack, IWorldAccessor world, BlockPos pos, ref EnumKilnModel kilnModel);
    }
    ...
    public interface IInKilnRendererSupplier
    {
        /// <summary>
        ...
        /// <param name="pos"></param>
        /// <returns></returns>
        IInKilnRenderer GetRendererWhenInKiln (ItemStack stack, BlockEntityKiln kiln, bool forOutputSlot);
        ...
        /// <param name="pos"></param>
        /// <returns></returns>
        EnumKilnModel GetDesiredKilnModel(ItemStack stack, BlockEntityKiln kiln, bool forOutputSlot);
    }



    public class BlockEntityKiln : BlockEntityOpenableContainer, IHeatSource
    {
        ...
        public float prevFurnaceTemperature = 35;

        // Current temperature of the furnace
        public float furnaceTemperature = 35;
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            {
                renderer = new KilnContentsRenderer(api as ICoreClientAPI, Pos);

                (api as ICoreClientAPI).Event.RegisterRenderer(renderer, EnumRenderStage.Opaque, "kiln");
        ...
        private void On500msTick(float dt)
        {
            ...
            if (Api.Side == EnumAppSide.Server && IsBurning && Api.World.Rand.NextDouble() > 0.2)
            {
                if (Api.World.BlockAccessor.GetRainMapHeightAt(Pos.X, Pos.Z) <= Pos.Y)   // It's more efficient to do this quick check before GetPrecipitation
                {
                    ...
                    if (rainLevel > 0.45 && Api.World.Rand.NextDouble() < rainLevel * 2)
                    {
                        Api.World.PlaySoundAt(new AssetLocation("game:sounds/effect/extinguish"), Pos.X + 0.5, Pos.Y, Pos.Z + 0.5, null, false, 16);

                        fuelBurnTime -= (float)rainLevel / 2f;

                        if (Api.World.Rand.NextDouble() < rainLevel / 2f || fuelBurnTime <= 0)
                        {
                            setBlockState("cold");
        ...
        public void heatInput(float dt)
        {
            ...
            {
                float f = (1 + GameMath.Clamp((furnaceTemperature - oldTemp) / 40, 3, 1.6f)) * dt;
            ...
            if (nowTemp >= meltingPoint)
            {
                ...
                inputStackCookingTime += GameMath.Clamp((int)(diff), 2, 35) * dt;
            }
            else
            {
        ...
        public void heatOutput(float dt)
        {
            ...
            if (oldTemp < furnaceTemperature)
            {
                float newTemp = changeTemperature(oldTemp, furnaceTemperature, 3 * dt);
        ...
        void SetTemp(ItemStack stack, float value)
        {
            ...
            if (inputStack.Collectible is ItemFood) // stop food being heated up :D other fixed maybe? or set the food to a high temp so it burn hahaha
            {
                for (int i = 0; i < inventory.CookingSlots.Length; i++)
                {
                    inventory.CookingSlots[i].Itemstack?.Collectible.SetTemperature(Api.World, inventory.CookingSlots[i].Itemstack, 0f);
                }
            }
            else
            {
        ...
        public EnumKilnModel CurrentModel { get; private set; }
        ...
        {
            Dictionary<string, MeshData> Meshes = ObjectCacheUtil.GetOrCreate(Api, "kiln-meshes", () => new Dictionary<string, MeshData>());
