...
namespace QptechFurniture.src
{
    public class BlockKiln : Block
    {
        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "kilnInteractions-" + Stage, () =>
            {
                ...
                return new WorldInteraction[]
                {
                    ...
                    new WorldInteraction()
                    {
                        ...
                        GetMatchingStacks = (wi, bs, es) => {
                            BlockEntityKiln bef = api.World.BlockAccessor.GetBlockEntity(bs.Position) as BlockEntityKiln;
        ...
        {
            BlockEntityKiln bef = api.World.BlockAccessor.GetBlockEntity(pos) as BlockEntityKiln;
            if (bef != null && bef.fuelSlot.Empty) return EnumIgniteState.NotIgnitablePreventDefault;
            //if (bef != null && bef.fuelSlot.Itemstack.Collectible.CombustibleProps?.BurnTemperature > 300) return EnumIgniteState.NotIgnitablePreventDefault; // stop people from using high or low grade fuel so we don't waste it :) 
        ...
        public bool TryConstruct(IWorldAccessor world, BlockPos pos, CollectibleObject obj, IPlayer player)
        {
            ...
            if (obj.Attributes?.IsTrue("kilnConstructable") != true) return false;

            if (stage == 5) return false;

            if (stage == 4 && world.BlockAccessor.GetBlock(pos.DownCopy()).Code.Path.Equals("stonebrick"))
            {
